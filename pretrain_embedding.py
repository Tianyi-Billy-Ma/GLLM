...
def preprocess_table(args):
    pretrain_dir = osp.join(args.processed_data_dir, args.dname, "pretrain")
    dataset = load_data(args)
    unique_tables, qas, tname2tid, qid2tid, qid2qname = generate_unique_tables(dataset)
    ...
    save_pickle(data, osp.join(pretrain_dir, "plaintext_data.pickle"))
...
def main(args):
    ...
    load_path = osp.join(pretrain_dir, "plaintext_data.pickle")
    if not osp.exists(load_path):
        data = preprocess_table(args)
    else:
    ...
    save_pickle(
        ...
        osp.join(pretrain_dir, "plaintext_data.pickle"),
    )
    ...
    dict_emb = {}
    for key, plaintext in passages.items():
        ids, embeddings = generate_embeddings(args, plaintext, model, tokenizer)
        dict_emb[key] = {"ids": ids, "embeddings": embeddings}

    save_pickle(dict_emb, osp.join(pretrain_dir, "embeddings.pickle"))
...
if __name__ == "__main__":
    args = parse_args()
    # args.LLMs_pretrain_model = "google-bert/bert-base-uncased"
    main(args)
    print("")
