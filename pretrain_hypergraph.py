from src import load_pickle, load_data, aug, contrastive_loss_node, BipartiteData
...
def construct_hypergraph(args, tables, embeddings):
    ...
    for t_idx, (key, val) in enumerate(tables.items()):
        ...
        x_s = torch.FloatTensor(embeddings[num_nodes:num_nodes + edge_index[0, :].max() + 1, :])
    ...
    assert (embeddings == torch.cat([hg.x_s for hg in hypergraph_list], dim=0).numpy()).all(), "Nodes mismatch"
    HG = DataLoader(hypergraph_list, batch_size=args.GNNs_batch_size, shuffle=False, drop_last=False)
    return HG
...
def main(args):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=args.GNNs_lr, weight_decay=args.GNNs_wd)
