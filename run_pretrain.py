...
from src.load_data import load_data, load_pickle, save_pickle, save_json
...
def main(args):
    ...
    emb_G_V = torch.zeros((num_tables, args.GNNs_hidden_dim)).to(device)
    emb_G_E = torch.zeros((num_tables, args.GNNs_hidden_dim)).to(device)
    emb_G_G = torch.zeros((num_tables, args.GNNs_hidden_dim)).to(device)
    with torch.no_grad():
        for batch_idx, batch in enumerate(HG):
            batch = batch.to(device)
            # emb_V_batch, emb_E_batch = model(batch)
            emb_V_batch, emb_E_batch = batch.x_s, batch.x_t
            emb_G_batch = batch.emb_G

            batch_G_V = scatter(emb_V_batch, batch.V2G, dim=0, reduce="mean")
            batch_G_E = scatter(emb_E_batch, batch.E2G, dim=0, reduce="mean")

            emb_G_V[batch.gid] = batch_G_V[batch.gid]
            emb_G_E[batch.gid] = batch_G_E[batch.gid]
            emb_G_G[batch.gid] = emb_G_batch

    emb_G_V = emb_G_V.cpu().detach().numpy()
    emb_G_E = emb_G_E.cpu().detach().numpy()
    emb_G_G = emb_G_G.cpu().detach().numpy()

    # ************************* Save Embeddings *************************

    tids = [f"table_{i}" for i in range(2100)]
    embs = [emb_G_V, emb_G_E, emb_G_G]
    save_dirs = [
        osp.join(args.root_dir, "output", "GNNs", "node"),
        osp.join(args.root_dir, "output", "GNNs", "table_from_he"),
        osp.join(args.root_dir, "output", "GNNs", "table_from_table"),
    ]

    for save_dir, emb in zip(save_dirs, embs):
        os.makedirs(save_dir, exist_ok=True)
        save_pickle(
            {"ids": tids, "embedding": emb},
    ...
    for save_dir in save_dirs:
        ...
        save_json(predictions, osp.join(save_dir, "predictions.json"))
        ...
        save_json(
            res,
            osp.join(save_dir, "result.json"),
        )
        print(f"{res}")


def evaluatation(args):
    # ************************* Evaluation *************************
    from experiment_retrieve_table import Retriever
    from src.preprocess import process_questions
    ...
    save_dirs = [
        osp.join(
            args.root_dir, "output", "GNNs", f"{args.LLMs_table_plaintext_format}_node"
        ),
        osp.join(
            args.root_dir, "output", "GNNs", f"{args.LLMs_table_plaintext_format}_he"
        ),
        osp.join(
            args.root_dir, "output", "GNNs", f"{args.LLMs_table_plaintext_format}_table"
        ),
    ]
...
if __name__ == "__main__":
    ...
    args.GNNs_reconstruct = True
    args.GNNs_regenerate_emb = True
    args.GNNs_pretrain_emb = True
    args.GNNs_epochs = 10
    args.GNNs_num_layers = 1
